...
OPS = {
  ...
  'conv_2x2' : lambda C, stride, affine: nn.Sequential(ReLUConvPool(C, C, 2, stride, 1, affine=affine)),
...
class ReLUConvPool(nn.Module):

  def __init__(self, C_in, C_out, kernel_size, stride, padding, affine=True):
    super(ReLUConvPool, self).__init__()
    self.op = nn.Sequential(
      nn.ZeroPad2d((padding, 0, padding, 0)),
